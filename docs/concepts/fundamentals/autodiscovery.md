...
[`COMPONENTS.dirs`](../../../reference/settings#django_components_lite.app_settings.ComponentsSettings.dirs) and 
[`COMPONENTS.app_dirs`](../../../reference/settings#django_components_lite.app_settings.ComponentsSettings.app_dirs)
directories.
...
tag needs to be registered with the [`ComponentRegistry`](../../../reference/api#django_components_lite.ComponentRegistry).

We use the [`@register`](../../../reference/api#django_components_lite.register) decorator for that:
...
- Components inside the auto-imported files still need to be registered with [`@register`](../../../reference/api#django_components_lite.register)
...
Autodiscovery can be disabled in the settings with [`autodiscover=False`](../../../reference/settings#django_components_lite.app_settings.ComponentsSettings.autodiscover).

### Manually trigger autodiscovery

Autodiscovery can be also triggered manually, using the [`autodiscover()`](../../../reference/api#django_components_lite.autodiscover) function. This is useful if you want to run autodiscovery at a custom point of the lifecycle:

```python
from django_components_lite import autodiscover

autodiscover()
```

To get the same list of modules that [`autodiscover()`](../../../reference/api#django_components_lite.autodiscover) would return,
but without importing them, use [`get_component_files()`](../../../reference/api#django_components_lite.get_component_files):
