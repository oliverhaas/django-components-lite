...
from django_components_lite import Component, ComponentVars
...
def gen_reference_api() -> None:
    """
    Generate documentation for the Python API of `django_components_lite`.

    This takes all public symbols exported from `django_components_lite`, except for those
    that are handled in other sections, like components, exceptions, etc.
    """
    module = import_module("django_components_lite")
...
def gen_reference_testing_api() -> None:
    """
    Generate documentation for the Python API of `django_components_lite.testing`.

    This takes all public symbols exported from `django_components_lite.testing`.
    """
    module = import_module("django_components_lite.testing")
...
def gen_reference_exceptions() -> None:
    """Generate documentation for the Exception classes included in the Python API of `django_components_lite`."""
    module = import_module("django_components_lite")
...
def gen_reference_settings() -> None:
    ...
    module = import_module("django_components_lite.app_settings")
...
def gen_reference_template_tags() -> None:
    ...
    These are discovered by looking at the files defined inside `django_components_lite/template_tags`.
    """
    tags_files = Path("./src/django_components_lite/templatetags").glob("*.py")
    tags_modules = [
        (p.stem, f"django_components_lite.templatetags.{p.stem}") for p in tags_files if not p.stem.startswith("_")
    ]
